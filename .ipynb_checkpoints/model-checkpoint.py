...
def check_auc(g_model_path, d_model_path, i):
    ...
    print("AUC: {0}, EER: {1}, EER_thr: {2}, F1_score: {3}".format(metrics.auc(fpr1,tpr1), EER1, eer_threshold1,f1_score))
...
class OGNet(nn.Module):
    ...
    def train(self, normal_class):
        ...
        fake = torch.ones([self.batch_size], dtype=torch.float32)
        valid = torch.zeros([self.batch_size], dtype=torch.float32)
